...
from contracts.ecosystem.genesis_collection import ZENDUCKS_COLLECTION
...
from contracts.shape.data_zenducks import (
...
@view
func get_local_index_{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, bitwise_ptr: BitwiseBuiltin*, range_check_ptr}(
    global_index: felt,
) -> (res: felt) {
    // For ducks, the collection ID is 3
    return ((global_index - ZENDUCKS_COLLECTION) / (2 ** 192) - 1, );
}


func _get_local_index{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, bitwise_ptr: BitwiseBuiltin*, range_check_ptr}(
    global_index: felt,
) -> felt {
    // For ducks, the collection ID is 3
    return (global_index - ZENDUCKS_COLLECTION) / (2 ** 192) - 1;
...
func shape_{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, bitwise_ptr: BitwiseBuiltin*, range_check_ptr}(
    ...
    return _shape(_get_local_index(global_index));
}
...
func check_shape_numbers_{
    ...
    with_attr error_message("Wrong number of shape items") {
        let (start, end) = _get_shape_offsets(_get_local_index(global_index));
        assert shape_len = (end - start) / ShapeItem.SIZE;
    }

    with_attr error_message("Wrong number of NFTs") {
        let (start, end) = _get_nft_offsets(_get_local_index(global_index));
    ...
    let (nft_start, _) = _get_nft_offsets(_get_local_index(global_index));
    let (data_address, __) = _get_shape_offsets(_get_local_index(global_index));
