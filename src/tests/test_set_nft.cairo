...
fn test_empty_mint() {
    ...
    let token_id = set_assembler.assemble_(
        default_owner(),
        0xfade,
        array![0xcafe],
        array![0xfade],
    ...
    assert(token_id == 0x1a61b367b44cb5cdc969ad212931f85c0dc5d31227cc1bdb8bf65238a722a6a, 'bad token id');
...
fn test_simple_mint_and_burn() {
    ...
    assert(token_id == 0x3fa51acc2defe858e3cb515b7e29c6e3ba22da5657e7cc33885860a6470bfc2, 'bad token id');
    assert(default_owner() == set_nft.owner_of(token_id.into()), 'bad owner');
    ...
    set_assembler.disassemble_(
        default_owner(),
        token_id,
        array![FTSpec { token_id: 1, qty: 1 }],
        array![]
    );
    assert(starknet::contract_address_const::<0>() == set_nft.owner_of(token_id.into()), 'bad owner');
...
fn test_simple_mint_and_burn_2() {
    ...
    let token_id = set_assembler.assemble_(
        ...
        array![
            ShapeItem { color: '#ffaaff', material: 1, x: 2, y: 4, z: -2 },
            ShapeItem { color: '#ffaaff', material: 1, x: 2, y: 4, z: -2 },
            ShapeItem { color: '#ffaaff', material: 1, x: 2, y: 4, z: -2 },
            ShapeItem { color: '#ffaaff', material: 1, x: 2, y: 4, z: -2 },
        ],
    ...
    assert(token_id == 0x2d4276d22e1b24bb462c255708ae8293302ff6b17691ed07f5057aee0d6eda3, 'bad token id');
    assert(default_owner() == set_nft.owner_of(token_id.into()), 'bad owner');
    assert(set_nft.balance_of(default_owner()) == 1, 'bad balance');
...
#[test]
#[available_gas(3000000000)]
#[should_panic(expected: ('Set still has briqs', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED'))]
...
fn test_simple_mint_attribute_ok() {
...

    {
        ...
        CreateCollectionData { collection_id: 1, params: 2, admin_or_system: 'check_shape_booklet'.try_into().unwrap() }.serialize(ref calldata);
        world.execute('create_collection', (calldata));
    }

    {
        let mut calldata: Array<felt252> = ArrayTrait::new();
        RegisterShapeVerifierData { attribute_id: 0x1, class_hash: briq_protocol::tests::shapes::test_shape_1::TEST_CLASS_HASH.try_into().unwrap() }.serialize(ref calldata);
    ...
    let token_id = set_assembler.assemble_(
        ...
        array![FTSpec { token_id: 1, qty: 4 }],
        array![
            ShapeItem { color: '#ffaaff', material: 1, x: 1, y: 4, z: -2 },
            ShapeItem { color: '#ffaaff', material: 1, x: 2, y: 4, z: -2 },
            ShapeItem { color: '#ffaaff', material: 1, x: 3, y: 4, z: -2 },
            ShapeItem { color: '#ffaaff', material: 1, x: 4, y: 4, z: -2 },
        ],
        array![0x1],
    );
    assert(token_id == 0x2d4276d22e1b24bb462c255708ae8293302ff6b17691ed07f5057aee0d6eda3, 'bad token id');
...
#[test]
#[available_gas(3000000000)]
#[should_panic(expected: ('u128_sub Overflow', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED'))]
...
#[test]
#[available_gas(3000000000)]
#[should_panic(expected: ('bad shape item', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED'))]
fn test_simple_mint_attribute_bad_shape_item() {
    ...
    let token_id = set_assembler.assemble_(
        ...
        array![
            ShapeItem { color: '#ffaaff', material: 1, x: 1, y: 4, z: -2 },
            ShapeItem { color: '#ffaaff', material: 1, x: 2, y: 4, z: -2 },
            ShapeItem { color: '#ffaaff', material: 1, x: 0, y: 4, z: -2 },
            ShapeItem { color: '#ffaaff', material: 1, x: 4, y: 4, z: -2 },
        ],
        array![0x1],
    );
...
#[test]
#[available_gas(3000000000)]
#[should_panic(expected: ('bad shape length', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED'))]
