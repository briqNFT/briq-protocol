...
use briq_protocol::felt_math::{FeltBitAnd, FeltDiv};
...
impl ShapePacking of StorePacking<ShapeItem, PackedShapeItem> {
    ...
    fn unpack(value: PackedShapeItem) -> ShapeItem {
        ShapeItem {
            color: (((value.color_material & (-1 - TWO_POW_64_MASK))).into()
                / Into::<felt252, u256>::into(TWO_POW_64))
                .try_into()
                .unwrap(),
            material: (value.color_material & TWO_POW_64_MASK).try_into().unwrap(),
