...
async def test_transfer():
    ...
    await contract.transfer_from(sender=0x11, recipient=0x12, token_id=0x124).invoke()
    
    assert await contract.owner_of(token_id=0x123).call() == (0x12,)
    assert await contract.balance_of(owner=0x11).call() == (2,)
    assert await contract.token_at_index(owner=0x12, index=0).call() == (0x123,)
    assert await contract.token_at_index(owner=0x11, index=0).call() == (0x123,)
    assert await contract.token_at_index(owner=0x11, index=1).call() == (0x125,)
